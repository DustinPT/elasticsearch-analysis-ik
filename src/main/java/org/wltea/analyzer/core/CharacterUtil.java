...
class CharacterUtil {
    ...
    static {
        String punctuations = ",.!?\"':;，。！？“”‘’：；、〔（）[]{}─-．《》〈〉·—_*×□/▲●～…→「」『』﹏\n";
        PUNCTUATIONS = punctuations.toCharArray();
    ...
    public static boolean isPunctuation(char ch) {
        return Arrays.binarySearch(PUNCTUATIONS, ch) >= 0;
    }
     ...
     * @return char
     */
    static char regularize(char input, boolean lowercase, boolean keepPunctuations) {
        if (keepPunctuations && isPunctuation(input)) {
            return input;
        }
        if (input == 12288) {
            input = (char) 32;
